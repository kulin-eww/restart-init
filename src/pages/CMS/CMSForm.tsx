...
const CMSForm = ({ action, cmsContent, title, role }: { action: "view" | "edit", cmsContent: any, title: string, role: string }) => {
        ...
        <>
            <div className="bg-layout-bg rounded-lg shadow-md px-6 py-4">
                <div className="flex justify-start items-center mb-2 gap-2">
                    ...
                    <div className="text-xl font-bold mb-2">{`${action === "view" ? "View" : "Edit"} ${role === "vendor" ? "Vendor" : role === "driver" ? "Driver" : role === "user" ? "Customer" : "Web"}'s ${title === "privacy_policy" ? "Privacy Policy" : title === "about_us" ? "About Us" : title === "terms_and_conditions" ? "Terms and Conditions" : ""}`}</div>
                </div>
                <hr />
                <div className="w-full mt-4 bg-bg-secondary p-4 rounded-xl">
                    <form onSubmit={handleSubmit} className="w-full mt-4 bg-bg-secondary p-4 rounded-xl">
